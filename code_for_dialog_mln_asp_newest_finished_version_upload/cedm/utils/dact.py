...
def _ParseAct(dact, user=True):

#     raw_act = __ParseAct(raw_act_text)
    raw_act = {'act' : dact.act, 'slots' : [[item.slot if item.slot is not None else 'this',item.val] for item in dact.items]}
    final_dialog_act = []
...
def _ParseActRel(dact, user=True):

#     raw_act = __ParseAct(raw_act_text)
    raw_act = {'act' : dact.act, 'slots' : [[item.slot+'#'+item.val,item.op] for item in dact.items]}
    final_dialog_act = []
...
def ParseAct(raw_act_text, user=True):
    if isinstance(raw_act_text, str) or isinstance(raw_act_text, unicode):
        final = []
        for act_text in raw_act_text.split("|") :
            try:
                final += _ParseAct(DActEntity.DiaActEntity(act_text), user=user)
            except RuntimeError:
                pass # add nothing to final if junk act recieved
        return final
    else: #assume raw_act_text is subclass of DAct
        return _ParseAct(raw_act_text, user=user)
