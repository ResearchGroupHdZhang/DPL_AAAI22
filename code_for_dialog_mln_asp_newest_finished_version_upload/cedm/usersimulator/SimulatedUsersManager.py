...
import cedm.usersimulator.UserModel as UserModel
import usersimulator.ErrorModel as ErrorModel
...
class DomainsSimulatedUser(object):
    ...
    '''
    def __init__(self, eType, error_rate):
        '''
        '''
        self.um = UserModel.UMCE(eType)
        self.error_simulator = ErrorModel.DomainsErrorSimulator(eType, error_rate)
        
        self.randomLearning = False
        if Settings.config.has_option("mogp_"+eType, "randomweightlearning"):
            self.randomLearning = Settings.config.getboolean("mogp_"+eType, "randomweightlearning")
            
        if self.randomLearning:
            import policy.morl.WeightGenerator as wg
            self.weightGen = wg.WeightGenerator(eType)
 
    def restart(self, otherDomainsConstraints, goal_with_rel):
        ...
        :returns: None
        '''
        self.um.init(otherDomainsConstraints, goal_with_rel) 
...
class SimulatedUsersManager(object):
    ...
    def __init__(self, error_rate):
        ...
        self.RELATION_PROBABILITY = 0.1
        if Settings.config.has_option('goalgenerator','relationprob'):
            self.RELATION_PROBABILITY = Settings.config.getfloat('goalgenerator','relationprob')
    ...
    def sample_domains(self):
        ...
        elif self.domainSampling == "all":
            self.using_domains = list(self.possible_domains)
    ...
    def restart(self):
        ...
        goal_with_rel = False
        if Settings.random.rand() < self.RELATION_PROBABILITY:
            goal_with_rel = True
        ...
        for dstring in self.using_domains: # doing this way to generate goals/behaviour in an order.
            ...
            self.simUserManagers[dstring].restart(otherDomainsConstraints, goal_with_rel)
            ...
            if self.CONDITIONAL_BEHAVIOUR:
                otherDomainsConstraints[dstring] = self.simUserManagers[dstring].um.goal.constraints
    ...
    def _user_act(self, sys_act):
        ...
        for dstring in self.using_domains:
            if dstring in self.uncompleted_domains:
                if len(self.visited_domains) and dstring not in self.visited_domains:
    ...
    def _addRelationToAgenda(self, targetDomain):
        ...
        #    compare goals, check whether slots are the same
            ...
            commonSlotsAndRelations[vdomain] = self._getCommonSlotsAndRelations(vGoal.constraints, tGoal.constraints) # currently only equals
        ...
        #        replace entry in agenda
                        for i in range(len(tAgenda.agenda_items)):
                            if tAgenda.agenda_items[i].contains_slot(slot):
                                ...
                                newAct.items = [DActEntity.DactItemEntity(slot, '=', slot, targetDomain, vdomain)]
    ...
    def _getCommonSlotsAndRelations(self, c1, c2):
        commonSlotsAndRelations = {}
        for item1 in c1:
            for item2 in c2:
                if item1.slot == item2.slot and item1.val == item2.val:
                    commonSlotsAndRelations[item1.slot] = '='
                    
        return commonSlotsAndRelations
