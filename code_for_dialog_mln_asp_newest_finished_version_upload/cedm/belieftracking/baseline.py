...
'''
baseline.py - Baseline Belief Tracker from DST for CEDM
=======================================================
...
.. note::
        ...
        here - just to keep PyDial simple. IT has been modified for the requirements of the CEDM.


.. seealso:: CUED Imports/Dependencies: 

    import :mod:`utils.Settings` |.|
    import :class:`cedm.belieftracking.BeliefTracker.BeliefTracker` |.|
...
#                             denied_goals[s].append(v)
            ...
            if slot not in denied_goals or value not in denied_goals[slot]:
    ...
    # dk449
        ...
        if lastInformedVenue != "" and lastInformedVenue is not None:
            method = "byname"
...
class RuleBasedTracker(BeliefTracker):
    ...
    def __init__(self,eType):
        super(RuleBasedTracker, self).__init__(eType)
    ...
    def _updateBelief(self, turn, entity):
        ...
        track = self._addTurn(turn, entity)
        return self._tobelief(entity, track)
    
    def _addTurn(self, turn, entity):
        '''
        Add turn info
    ...
    def _tobelief(self, entity, track):
        '''
        ...
        '''
        prev_belief = entity.belief
        belief = {}
        for slot in Ontology.global_ontology.get_informable_slots_and_values(entity.eType):
            if slot in track['goal-labels']:
                infom_slot_vals = Ontology.global_ontology.get_informable_slot_values(entity.eType,slot)
...
#                 else:
        ...
        belief['method'] = dict.fromkeys(Ontology.global_ontology.get_method(entity.eType), 0.0)
        for v in track['method-label']:
            belief['method'][v] = track['method-label'][v]
        belief['discourseAct'] = dict.fromkeys(Ontology.global_ontology.get_discourseAct(entity.eType), 0.0)
        for v in track['discourseAct-labels']:
            belief['discourseAct'][v] = track['discourseAct-labels'][v]
        belief['requested'] = dict.fromkeys(Ontology.global_ontology.get_requestable_slots(entity.eType), 0.0)
        for v in track['requested-slots']:
            belief['requested'][v] = track['requested-slots'][v]

        return {'beliefs': belief}
    
    def _setUpEntity(self, obj):
        if not hasattr(obj, 'lastInformedVenue'):
            obj.lastInformedVenue = ''
...
class BaselineTracker(RuleBasedTracker):
    ...
    """
    def __init__(self, eType):
        super(BaselineTracker, self).__init__(eType)
        self.restart()
        
    def _addTurn(self, turn, obj):
        '''
        Add turn info
        ...
        for score, uact in slu_hyps :
            informed_goals, denied_goals, requested, method, discourseAct, obj.lastInformedVenue = labels(uact, mact, obj.lastInformedVenue)
            # requested
    ...
    def _setUpEntity(self, entity):
        super(BaselineTracker, self)._setUpEntity(entity)
        if not hasattr(entity, 'hyps'):
            entity.hyps = {"goal-labels":{}, "goal-labels-joint":[], "requested-slots":{}, "method-label":{}, "discourseAct-labels":{}}
...
class FocusTracker(RuleBasedTracker):
    ...
    """
    def __init__(self,eType):
        super(FocusTracker, self).__init__(eType)
        self.restart()
    ...
    def _addTurn(self, turn, obj):
        ...
        for slot in set(this_u.keys() + hyps["goal-labels"].keys()) :
            ...
            for value in this_u[slot] :
                if value==slot:
                    logger.error('input for {} contains relation: {}'.format(obj, turn))
...
#         self.hyps = {"goal-labels":{},"method-label":{}, "requested-slots":{}}
    ...
    def _setUpEntity(self, entity):
        super(FocusTracker, self)._setUpEntity(entity)
        if not hasattr(entity, 'hyps'):
            entity.hyps = {"goal-labels":{},"method-label":{}, "requested-slots":{}}
