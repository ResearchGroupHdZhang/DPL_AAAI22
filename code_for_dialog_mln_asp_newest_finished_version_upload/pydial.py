...
def initialise(configId, config_file, seed, mode, trainerrorrate=None, trainsourceiteration=None,
    ...
    # added by zzc
    seed = Settings_krr.init(config_file, seed)
    ...
    # TODO @Jerry, set n_in for this.
    if isSingleDomain:
        if Settings.config.has_section("dqnpolicy"):
            if domain == 'CamRestaurants':
                Settings.config.set("dqnpolicy", 'n_in', 180)
                ...
                Settings.config.set("dqnpolicy", 'n_in', 111 + 216)
            elif domain == 'SFHotels':
                Settings.config.set("dqnpolicy", 'n_in', 438)
            elif domain == 'Laptops6':
                Settings.config.set("dqnpolicy", 'n_in', 180)
            elif domain == 'TV':
                Settings.config.set("dqnpolicy", 'n_in', 188)
    ...
    if mode == "train":
        if gnumtrainbatches > 1:
            ...
            logfile = "%s-%s%02d.%d-%d.train.log" % (
                configId, seed_string, gtrainerrorrate, gtrainsourceiteration + 1, enditeration)
        else:
        ...
        if testenderrorrate:
            ...
            if type(iteration) == str:
                ...
                logfile = "%s-%s%02d.%d.eval.%02d.log" % (
                    configId, seed_string, gtrainerrorrate, iteration, gtesterrorrate)
    elif mode == "chat":
    ...
    # # added by zzc
    Settings_krr.random_krr.seed(int(seed))
...
def trainBatch(domain, configId, trainerr, ndialogs, source_iteration, seed=None):
    ...
    # # added
    # simulator.slot_inference_model
...
def plotTrain(dname, rtab, stab, ttab, block=True, saveplot=False):
    font = {
        'weight': 'bold',
        'size': 20}
    plt.rc('font', **font)
    ...
    plt.figure(gplotnum, figsize=[20, 12])
    ...
    for policy in policylist:
        ...
        if len(tab['x']) < 2:
            ...
            plt.plot(tab['x'], tab['y'], label=policy)
            plt.fill_between(tab['x'], tab['y'] - tab['var'], tab['y'] + tab['var'], alpha=0.2)
        ...
        for policy_key in result:
            ...
            for key in result[policy_key]:
                if not key in averaged_result[general_policy_key]:
                    ...
                    averaged_result[general_policy_key][key].append(np.array(result[policy_key][key]))
        for policy_key in averaged_result:
            for key in averaged_result[policy_key]:
                if key == 'y':
                    std = np.std(averaged_result[policy_key][key], axis=0)
                averaged_result[policy_key][key] = np.mean(averaged_result[policy_key][key], axis=0)
        # add y_std into the result
        averaged_result[policy_key]['y_std'] = std
...
def train_command(configfile, seed=None, train_error_rate=None, train_source_iteration=None,
                  num_train_batches=None, train_dialogs_per_batch=None, train_domains=None, db_prefix=None):
    ...
    try:
        if seed and seed.startswith('('):
            ...
            for seed in seeds:
                ...
                train_command(configfile, seed=seed, train_error_rate=train_error_rate,
                              train_source_iteration=train_source_iteration,
                              num_train_batches=num_train_batches, train_dialogs_per_batch=train_dialogs_per_batch,
                              train_domains=train_domains, db_prefix=db_prefix)

        else:
            configId = getConfigId(configfile)
            if seed:
                seed = int(seed)
            initialise(configId, configfile, seed, "train", trainerrorrate=train_error_rate,
                       trainsourceiteration=train_source_iteration, numtrainbatches=num_train_batches,
                       traindialogsperbatch=train_dialogs_per_batch, traindomains=train_domains, dbprefix=db_prefix)
            ...
            logger.results("*** Training complete. log: %s - final policy is %s-%02d-%02d" % (
                logfile, configId, gtrainerrorrate, i + 1))
            ...
            if poldirpath.isdir():
                ...
                if isSingleDomain:
                    if policyname in policynamelist:
                        ...
                        logger.results(
                            "*** Testing complete. logfile: %s - policy %s evaluated" % (logfile, policyname))
                    else:
    ...
    try:
        ...
        if ncurves > 0:
            ...
            for domain in domains:
                ...
                if not noplot:
                    plotTrain(datasetname + '-' + domain, rtab, stab, ttab, block=block, saveplot=saveplot)
