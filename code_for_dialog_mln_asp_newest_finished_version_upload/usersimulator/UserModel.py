...
# added by zzc
from utils import Settings_krr

logger = ContextLogger.getLogger('')

# slot values used in goal sampling.
import os
import dill

# TODO: @Jerry, load different inference model. 
# FIXME: 
# Update: this should not affect existing CR config.
# if not Settings.config.has_option('mln', "use_custom_inference_model") or Settings.config.getboolean('mln', "use_custom_inference_model"):

# if Settings.config.has_option('mln', "use_custom_inference_model"):
#     domains = Settings.config.getboolean("GENERAL", "domains")

# import pydial.record_domain as record_domain 
...
# domain = ""
# bayesian_inference_path = ""
# slot_inference_model = None

# # choose domain before running
# for line in open("domain_cr.txt"):
#     domain = line

# # for line in open("domain_ch.txt"):
# #     domain = line
...
#     bayesian_inference_path = os.path.abspath(
#         os.path.join(os.path.dirname(__file__), '..', 'MLN/bn_inference.pkl'))
# elif domain == "CamHotels":
#     bayesian_inference_path = os.path.abspath(
#         os.path.join(os.path.dirname(__file__), '..', 'MLN_CH/bn_inference.pkl'))

# slot_inference_model = dill.load(open(bayesian_inference_path, 'rb'))
...
class UMGoal(object):
    ...
    def __init__(self, patience, domainString):
        ...
        # Settings.set_seed(int(self.randomseed))
        # # Settings.random.seed = int(self.randomseed)
...
class GoalGenerator(object):
    ...
    def __init__(self, dstring):
        ...
        # # load pickle file generated by bayesian network to sample
        # # slot values used in goal sampling.
        # import os
        # import dill
        ...
        #         os.path.join(os.path.dirname(__file__), '..', 'MLN/bn_inference.pkl')
        #     )
        #     self.slot_inference_model = dill.load(open(bayesian_inference_path, 'rb'))
...
#            self.NO_REQUESTS_WITH_VALUE_NONE
    ...
    def _init_consts_requests(self, goal, um_patience):
        ...
        # TODO: @Jerry
        # sampling external factors and slot and slot values from bayesian network
        # import SimulationSystem.slot_inference_model as slot_inference_model
        import Simulate 
        if not Settings.config.has_option('mln', "use_custom_inference_model") or Settings.config.getboolean('mln', "use_custom_inference_model"):
            # external_factors, sampled_slots = self.slot_inference_model.sample_next_goal()
            # external_factors, sampled_slots = slot_inference_model.sample_next_goal()
            external_factors, sampled_slots = Simulate.slot_inference_model.sample_next_goal()
            sampled_slots = sampled_slots[:min(self.MAX_CONSTRAINTS, len(sampled_slots))]
            # fill in constraints for the selected slots
            for slot, slot_val in sampled_slots:
                goal.add_const(slot, slot_val)
        else:
            # Get a list of slots that are valid for this task.
            valid_const_slots = Ontology.global_ontology.getValidSlotsForTask(self.dstring)
        ...
        if self.MIN_REQUESTS == self.MAX_REQUESTS:
            ...
            # n = Settings.random.randint(low=self.MIN_REQUESTS-1,high=self.MAX_REQUESTS)
                    ...
                    # import numpy as np
            # n = np.random.randint(low=self.MIN_REQUESTS-1,high=self.MAX_REQUESTS)
            n = Settings_krr.random_krr.randint(low=self.MIN_REQUESTS-1,high=self.MAX_REQUESTS)            
        ...
        if n > 0 and len(valid_req_slots) >= n:   # ie more requests than just 'name'
            ...
            # choosen = np.random.choice(valid_req_slots, n,replace=False)
            choosen = Settings_krr.random_krr.choice(valid_req_slots, n,replace=False)
...
class UM(object):
    ...
    def init(self, otherDomainsConstraints):
        ...
        if self.sample_patience:
            ...
            self.max_patience = Settings_krr.random_krr.randint(self.sample_patience[0], self.sample_patience[1])
