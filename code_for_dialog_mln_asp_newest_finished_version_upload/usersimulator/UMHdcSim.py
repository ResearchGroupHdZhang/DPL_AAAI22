...
#             print "dialogue act", dap.act, dap.items
            ...
            while self.agenda.size() > 0 and \
                ...
                # Change 'hello()' response with no other appended constraint information,
                # i.e. hello(type="placetostay")
                # from 'hello(type="placetostay")' => 'hello()'
                # if dap.act == 'hello':
                #     break
    ...
    def _receive_hello(self, sys_act, goal):
        if not len(sys_act.items):
            ...
            # Comment out the if statement so that simulated users will be certain to give 'hello()' response
            # whenever the sys/agent says 'hello().'
