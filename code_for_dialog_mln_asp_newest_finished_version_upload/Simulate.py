...
class SimulationSystem(object):
    ...
    def __init__(self, error_rate):
        ...
        # added
        import dill
        global domain, bayesian_inference_path, slot_inference_model
        # self.domain = ""
        # self.bayesian_inference_path = ""
        # self.slot_inference_model = None
        
        if domain == "" and Settings.config.has_option('GENERAL', "domains"):
            domain = Settings.config.get("GENERAL", "domains")

        if bayesian_inference_path == "":
            if domain == "CamRestaurants":
                bayesian_inference_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), 'MLN_CR/bn_inference.pkl'))
            elif domain == "CamHotels":
                bayesian_inference_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), 'MLN_CH/bn_inference.pkl'))

        if slot_inference_model is None:
    ...
    def run_dialogs(self, numDialogs):
        ...
        # # added
        # import numpy as np
        # np.random.shuffle(self.slot_inference_model.samples)

        # saved
        # from usersimulator import UserModel
        # import numpy as np
        # np.random.shuffle(UserModel.slot_inference_model.samples)        

        global slot_inference_model
        import numpy as np
        np.random.shuffle(slot_inference_model.samples)     
