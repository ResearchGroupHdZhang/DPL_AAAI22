...
randomCount_krr = 0
root_krr = ''
global_currentturn_krr = None
global_numiter_krr = None
global_sess_krr = None
...
def init(config_file, seed=None):
    ...
    # -----------------------------------------
    if seed is None:
        # no seed given at cmd line (the overriding input), so check config for a seed, else use None (which means
        # use clock).
        if config_krr.has_option("GENERAL", 'seed'):
            seed = config_krr.getint("GENERAL", "seed")
    seed = set_seed(seed)
...
def load_config(config_file):
    ...
    if config_file is not None:
        try:
            config_krr = ConfigParser.ConfigParser()
            config_krr.read(config_file)
...
def load_root(rootIn=None):
    """
    Root is the location (ie full path) of the cued-python repository. This is used when running things on the grid
    (non local machines in general).
    """
    global root_krr
    root_krr = ''

    if config_krr is not None:
        if config_krr.has_option("GENERAL", 'root'):
            root_krr = config_krr.get("GENERAL", 'root')
...
def set_seed(seed):
    """
    Initialise np random num generator
    ...
    if seed is None:
        ...
        seed = random1.randint(1000000001)
    random_krr = nprandom.RandomState(seed)

    return seed


def randomProfiling(t=None):
    global randomCount_krr
    randomCount_krr += 1
    print "Random access: {} from class {}".format(randomCount_krr, t)
    return random_krr
...
def locate_file(filename):
    ...
    if os.path.exists(filename):
        ...
        return root_krr + filename  # note this may or may not exist
