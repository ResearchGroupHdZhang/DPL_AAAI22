...
class Evaluator(object):
    ...
    def __init__(self, domainString):
        ...
        if Settings.config.has_option('GENERAL', "mln_training_data_size"):
            ...
            self.mln_training_data_size = Settings.config.get('GENERAL', "mln_training_data_size")
        ...
        if Settings.config.has_option('eval', 'asp_diff'):
            self.incomplete_asp = Settings.config.getboolean('eval', 'asp_diff')

        self.policytype = None 
        if Settings.config.has_option("policy", "policytype"):
            self.policytype = Settings.config.get("policy", "policytype")

        self.knowledge_bool = None
        if Settings.config.has_option('GENERAL', "knowledge_bool"):
            self.knowledge_bool = Settings.config.getboolean('GENERAL', "knowledge_bool")
    ...
    def finalReward(self, finalInfo):
        ...
        '''
        if self.num_turns > 0:
            final_reward, final_asp_reward = self._getFinalReward(finalInfo)
            self.total_reward += final_reward
                
            self.rewards.append(self.total_reward)

            # # added by zzc
            # self.ori_outcomes.append(self.outcome)

            if final_reward == final_asp_reward:
                self.outcomes.append(self.outcome)
            else:
                # if self.outcome:
                
                # note: When the result of the original reasoning does not conform to the constraints of ASP, it is judged as failure
                self.outcomes.append(False)

                # note: If the original result failed, don't have to worry about the result in ASP mode
                # else:
    ...
    def print_summary(self):
        ...
        if num_dialogs:
            ...
            if self.mln_training_data_size:
                # if not os.path.exists('./output-mln'):
                #     os.mkdir('./output-mln')
                # with open('./output-mln/output-{}-{}.txt'.format(
                #         self.randomseed, self.mln_training_data_size), 'a+') as f:
                #     f.write(str(self.outcomes) + '\n')


                if not os.path.exists('./output-mln'):
                    os.mkdir('./output-mln')
                with open('./output-mln/output-{}-{}-{}-{}.txt'.format(self.randomseed, self.mln_training_data_size, self.policytype, self.domainString), 'a+') as f:
                    f.write(str(self.outcomes) + '\n') 

                if not os.path.exists('./output-mln-reward'):
                    os.mkdir('./output-mln-reward')
                with open('./output-mln-reward/output-mln-reward-{}-{}-{}-{}.txt'.format(self.randomseed, self.mln_training_data_size, self.policytype, self.domainString), 'a+') as f:
                    f.write(str(self.rewards) + '\n')  

                if not os.path.exists('./output-mln-turns'):
                    os.mkdir('./output-mln-turns')
                with open('./output-mln-turns/output-mln-turns-{}-{}-{}-{}.txt'.format(self.randomseed, self.mln_training_data_size, self.policytype, self.domainString), 'a+') as f:
                    f.write(str(self.turns) + '\n')   


                # if not os.path.exists('./output-mln-origin'):
                #     os.mkdir('./output-mln-origin')
                # with open('./output-mln-origin/output-mln-origin-{}-{}.txt'.format(
                #         self.randomseed, self.mln_training_data_size), 'a+') as f:
                #     f.write(str(self.ori_outcomes) + '\n')

                # if not os.path.exists('./output-mln-origin'):
                #     os.mkdir('./output-mln-origin')
                # with open('./output-mln-origin/output-mln-origin-{}-{}-2.txt'.format(
                #         self.randomseed, self.mln_training_data_size), 'a+') as f:
                #     f.write(str(self.ori_outcomes) + '\n')
                ...
                # if not os.path.exists('./output-mln-origin'):
                #     os.mkdir('./output-mln-origin')
                # with open('./output-mln-origin/output-mln-origin-{}-{}-1-false.txt'.format(
                #         self.randomseed, self.mln_training_data_size), 'a+') as f:
                #     f.write(str(self.ori_outcomes) + '\n')
                ...
                if self.asp_diff:
                    ...
                    if self.knowledge_bool is None:

                        if not os.path.exists('./output-asp'):
                            os.mkdir('./output-asp')
                        with open('./output-asp/output-asp-{}-{}-{}-{}-{}.txt'.format(
                                self.randomseed, self.inaccurate_asp, self.incomplete_asp, self.policytype, self.domainString), 'a+') as f:
                            f.write(str(self.outcomes) + '\n')

                        if not os.path.exists('./output-asp-reward'):
                            os.mkdir('./output-asp-reward')
                        with open('./output-asp-reward/output-asp-reward-{}-{}-{}-{}-{}.txt'.format(
                                self.randomseed, self.inaccurate_asp, self.incomplete_asp, self.policytype, self.domainString), 'a+') as f:
                            f.write(str(self.rewards) + '\n')

                        if not os.path.exists('./output-asp-turns'):
                            os.mkdir('./output-asp-turns')
                        with open('./output-asp-turns/output-asp-turns-{}-{}-{}-{}-{}.txt'.format(
                                self.randomseed, self.inaccurate_asp, self.incomplete_asp, self.policytype, self.domainString), 'a+') as f:
                            f.write(str(self.turns) + '\n')

                    else:
                        if not os.path.exists('./output-cp-knowledge-bool'):
                            os.mkdir('./output-cp-knowledge-bool')
                        with open('./output-cp-knowledge-bool/output-cp-knowledge-bool-{}-{}-{}-{}.txt'.format(
                                self.randomseed, self.knowledge_bool, self.policytype, self.domainString), 'a+') as f:
                            f.write(str(self.outcomes) + '\n')                                

                else:
                    if not os.path.exists('./output'):
                        os.mkdir('./output')
                    with open('./output/output-{}-{}-{}-{}-{}.txt'.format(self.randomseed, self.add_knowledge, self.add_asp, self.policytype, self.domainString), 'a+') as f:
                        f.write(str(self.outcomes) + '\n')  
                    

                    if not os.path.exists('./output-turns'):
                        os.mkdir('./output-turns')
                    with open('./output-turns/output-turns-{}-{}-{}-{}-{}.txt'.format(self.randomseed, self.add_knowledge, self.add_asp, self.policytype, self.domainString), 'a+') as f:
                        f.write(str(self.turns) + '\n')


                    if not os.path.exists('./output-reward'):
                        os.mkdir('./output-reward')
                    with open('./output-reward/output-reward-{}-{}-{}-{}-{}.txt'.format(self.randomseed, self.add_knowledge, self.add_asp, self.policytype, self.domainString), 'a+') as f:
                        f.write(str(self.rewards) + '\n')
                ...
                # if not os.path.exists('./output-origin'):
                #     os.mkdir('./output-origin')
                # with open('./output-origin/output-origin-{}-{}-{}.txt'.format(self.randomseed, self.add_knowledge,
                #                                                      self.add_asp), 'a+') as f:
                #     f.write(str(self.ori_outcomes) + '\n')
