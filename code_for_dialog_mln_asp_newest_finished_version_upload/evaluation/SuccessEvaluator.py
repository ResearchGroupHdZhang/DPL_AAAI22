...
# from CamHotels_Reasoning.knowledge_testing import reason_LP
from ASP.knowledge_testing import reason_LP
...
class ObjectiveSuccessEvaluator(Evaluator):
    ...
    def __init__(self, domainString):
        ...
        self.add_asp = False
        if Settings.config.has_option('eval', 'add_asp'):
            self.add_asp = Settings.config.getboolean('eval', 'add_asp')
        ...
        self.inaccurate_asp = False
        if Settings.config.has_option('eval', 'inaccurate_asp'):
            self.inaccurate_asp = Settings.config.getboolean('eval', 'inaccurate_asp')
        self.incomplete_asp = False
        if Settings.config.has_option('eval', 'incomplete_asp'):
            self.incomplete_asp = Settings.config.getboolean('eval', 'incomplete_asp')

        self.asp_diff = False
        if Settings.config.has_option('eval', 'asp_diff'):
            self.asp_diff = Settings.config.getboolean('eval', 'asp_diff')


        # added by zzc
        if domainString == "CamRestaurants":
            self.perfect_reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
                                'ASP-CR/CamHotels_rules.lp'))
            
            if self.inaccurate_asp and self.incomplete_asp:
                self.reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
                                'ASP-CR/CamHotels_inaccurate_incomplete_rules.lp')
                )
            elif self.inaccurate_asp:
                self.reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
                                'ASP-CR/CamHotels_inaccurate_rules.lp')
                )
            elif self.incomplete_asp:
                self.reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
                                'ASP-CR/CamHotels_incomplete_rules.lp')
                )
            else:
                self.reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
            ...
            self.perfect_reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
                                'ASP/CamHotels_rules.lp'))
            
            if self.inaccurate_asp and self.incomplete_asp:
                self.reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
                                'ASP/CamHotels_inaccurate_incomplete_rules.lp')
                )
            elif self.inaccurate_asp:
                self.reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
                                'ASP/CamHotels_inaccurate_rules.lp')
                )
            elif self.incomplete_asp:
                self.reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
                                'ASP/CamHotels_incomplete_rules.lp')
                )
            else:
                self.reasoning_rule_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..',
        ...
        #                      'ASP/CamHotels_rules.lp')
        #     )
        if not os.path.exists(self.reasoning_rule_path):
            print(self.reasoning_rule_path, " doesn't exist")
            exit(1)
...
#             is_valid_list.append(is_valid)
    ...
    def _isValidVenue_final(self, name, constraints):

        # # add external factors sampling
        ...
        constraints2, constraints3 = None, None
        new_eval, prev_eval = None, None
        # flag for finding the hotel
        found = False
        ...
        if isinstance(constraints, list):
            ...
            # remove the recommended name
            for const in constraints2:
                if const.slot == 'name':
                    constraints2.remove(const)
                    
            # get results from logic program
            results = reason_LP(external_factors, [self.reasoning_rule_path])
            # ASP
            if self.add_asp and self.add_knowledge:
                for result in results:
                    asp_constraints = []
                    sub_result = [str(res) for res in result]
                    for info in sub_result:
                        slot = info[info.index('_') + 1: info.index('(')]
                        slot_val = info[info.index('(') + 1: info.index(')')]
                        asp_constraints.append(DactItem(slot, '=', slot_val))
    
                    hotels = Ontology.global_ontology.entity_by_features(self.domainString,
                                                                         constraints2 + asp_constraints)
                    # update_hasparking is related to bad weather
                    # incomplete asp removed the rule related to update_hasparking
                    if any(hotels):
                        found = True
                        constraints3 = constraints2 + asp_constraints
                        break
        ...
        if prev_eval is None:
            entities = Ontology.global_ontology.entity_by_features(self.domainString, constraints2)
            prev_eval = any(entities)
            
        # get the perfect knowledge
        perfect_results = reason_LP(external_factors, [self.perfect_reasoning_rule_path])
        # revised evaluation
        if self.add_asp:
            if found:
                # ASP mode
                # constraints3 = constraints2 + selected_answer_set constraints
                rec_entity = Ontology.global_ontology.entity_by_features(self.domainString, constraints3)[0]
            else:
                # if no such entity matches one of the answer sets with given knowledge
                return prev_eval, False
        else:
            rec_entity = Ontology.global_ontology.entity_by_features(self.domainString, constraints2)
            if rec_entity:
                rec_entity = rec_entity[0]
            else:
                return prev_eval, False
        
        # check rec_entity matches one of the possible answer sets
        for result in perfect_results:
            sub_result = [str(res) for res in result]
            match_knowledge = True
            for info in sub_result:
                slot = info[info.index('_') + 1: info.index('(')]
                slot_val = info[info.index('(') + 1: info.index(')')]
                if rec_entity[slot] != slot_val:
                    match_knowledge = False
                    break
            # find a answer set that matches rec_entity
            if match_knowledge:
                new_eval = True
                break
        # doesn't find one answer set that matches rec_entity
        if not new_eval:
            new_eval = False

    
        # modified by zzc
        # let tne setting of ASP mode is the same as tne setting of main mode 

        # # note: In order to better compare the role of completeness and accuracy in the ASP model
        # # ASP needs to be validated in all Trials
        # # for asp_diff experiment 
        # if self.asp_diff:
        #     return prev_eval, new_eval


        # added by zzc
        # note: ASP inference results are enabled once for every four trials
        # for mln_diff experiment and main experiment 
        from ASP.knowledge_testing import Save_factors
        NUM_DIALOGS = Save_factors.NUM_DIALOGS
        # @Jerry TODO: disable and False here
        # if NUM_DIALOGS % 3 == 0:

        # added by zzc
        print 'NUM_DIALOGS: ' + str(NUM_DIALOGS)
        print '\n' 
        print prev_eval
        print '\n'  
    ...
    def _getFinalReward(self, finalInfo):
            ...
            if 'usermodel' in finalInfo: # from user simulator
                ...
                if um is None:
                    ...
                    if None not in requests.values():
                        valid_venue, is_valid_under_rule = self._isValidVenue_final(requests['name'], self.user_goal)
                        if valid_venue:
                            self.outcome = True
                            logger.dial(
        ...
        original_rewards = self.outcome * self.successReward - (not self.outcome) * self.failPenalty
        if is_valid_under_rule is None:
            return original_rewards, original_rewards
        return self.outcome * self.successReward - (not self.outcome) * self.failPenalty, \
               is_valid_under_rule * self.successReward - (not is_valid_under_rule) * self.failPenalty
