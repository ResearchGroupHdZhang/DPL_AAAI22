...
def getInformByConstraints(constraints, domainString, lastInformedVenue):
    ...
    # if database doesn't have any recommendations that match given constraints, then
    # we return 'inform(name=none, {constraints})'
    if len(entities) == 0:
        return getInformNoneVenue(constraints)
    else:
        # if we find a recommendation/venue that matches previously informed one,
        # then we use the same entity/venue/recommendation, else use the first one.
        ret_ent = entities[0]
        for ent in entities:
            if ent['name'] == lastInformedVenue:
                ret_ent = ent
                break
        # combining constraints with currently selected entities
...
def getInformEntity(accepted_values, ent):
    ...
    # combining accepted_values with entity
    for slot in acceptance_keys:
        # for slots that are not 'name'
        if slot != 'name':
            value = accepted_values[slot]
            if value == 'dontcare' and slot in ent and ent[slot] != "not available":
                feats[slot] = ent[slot]
            else:
                # add slot into features if
                # 1. slot's value is not 'dontcare'
                # 2. slot doesn't exist in currently selected entity
                # 3. slot exists in entity but not value assigned
                if slot in ent:
                    feats[slot] = ent[slot]
                else:
