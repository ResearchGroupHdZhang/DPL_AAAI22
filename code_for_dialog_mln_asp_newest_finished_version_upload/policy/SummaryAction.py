...
class SummaryAction(object):
    ...
    def getInformByConstraints(self, belief):
        # using 'system requestable' slots, and pick each slot's top {slot, (value, belief)} dict (!= 'NONE')
        # by comparing its belief with threshold: 'auto' to append into 'accepted_values'
        accepted_values = SummaryUtils.getTopBeliefs(belief, domainString=self.domainString)
        # convert accepted_values from {slot: (value, belief)} to {slot: value}
        constraints = SummaryUtils.get_constraints(accepted_values)
        # look into the database to find entities given constraints.
        # if no entities are found, then return 'inform(name=none, constraints)'
        # else check whether LastInformedVenue is in found entities, if so, keep it and combine it with constraints
        # if not, then use the first entity with constraints info added.
    ...
    def getInformByName(self, belief):
        # checking requested slots, if a request slot has a requesting probability higher than requesting_threshold
        # then add it into requests_slots
        requested_slots = SummaryUtils.getRequestedSlots(belief)
        # find the name of the entity with the highest belief/prob
        name = SummaryUtils.getTopBelief(belief['beliefs']['name'])[0]
        if name == '**NONE**':
            name = belief['features']['lastInformedVenue']
        # combine entity 'name=name' with requested_slots info.
        # 1. if there exists at least one entity with 'name=name', pick the first entity
        # 2. if there doesn't exits one entity, and given name is 'None', randomly use a name to find entities and pick
        #    the first entity
        #    proceeded by both 1 and 2:
        #    A. if requested_slots is empty
        #       A1. if 'type' in the entity, add 'type' into 'inform'
        #       A2. else, get 'requestable slots' (excluding 'name'), randomly pick a random number of them to add
        #    B. if requested_Slots is not empty
        #       B1. pick max_num_features randomly (excluding 'name' and 'location') to add into 'inform'
        # 2.1 if there doesn't exist one entity, and given name is not None,
        #     then return 'inform(name=none, constraints that are not 'dontcare')'
        return SummaryUtils.getInformRequestedSlots(requested_slots, name, self.domainString)

    def getInformAlternatives(self, belief):
        self.alternatives_requested = False
        # returns all venues informed previously
        informedVenueSinceNone = set(belief['features']['informedVenueSinceNone'])
        accepted_values = SummaryUtils.getTopBeliefs(belief, domainString=self.domainString)
        # by converting accepted_values to constraints used to find entities, check each informed venue one by one
        # whether it is in a 'prohibited list == informedVenueSinceNone'.
        # 1. if not, 'getInformEntity' is used to combine constraints info
        #    with the first entity that is not in 'informedVenueSinceNone'.
        # 2. if all informed venues/entities are in 'informedVenueSinceNone', 'getInformNoMoreVenues' is called to
        #    combine info with constraints (converted from accepted values) and informedVenues to return
        # 'inform(name=none, name!=informedVenues, constraints)'
