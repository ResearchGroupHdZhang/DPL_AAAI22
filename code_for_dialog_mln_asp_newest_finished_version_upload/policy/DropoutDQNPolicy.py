...
'''
DropoutDQNPolicy.py - Dropout deep Q network policy
==================================================

Copyright CUED Dialogue Systems Group 2015 - 2018

Implementation of concrete algorithm based on Gal et al 2017. You can choose the probability
of dropping units through keep_prob parameter. Dropout is not used at the testing time.
...
class DropoutDQNPolicy(Policy.Policy):
    ...
    def __init__(self, in_policy_file, out_policy_file, domainString='CamRestaurants', is_training=False):
        ...
        self.keep_prob = 1.0
        if cfg.has_option('dqnpolicy_' + domainString, 'keep_prob'):
            self.keep_prob = cfg.getfloat('dqnpolicy_' + domainString, 'keep_prob')
    ...
    def record(self, reward, domainInControl=None, weight=None, state=None, action=None):
        ...
        cur_action_q = self.dqn.predict(cur_cState, self.epsilon_start) #alpha
        cur_target_q = self.dqn.predict_target(cur_cState, 1.0)
    ...
    def nextAction(self, beliefstate):
        ...
        if self.exploration_type == 'e-greedy':
            # epsilon greedy
            if self.is_training:
                action_Q = self.dqn.predict(np.reshape(beliefVec, (1, len(beliefVec))), self.keep_prob)
                ...
                action_Q = self.dqn.predict(np.reshape(beliefVec, (1, len(beliefVec))), 1.0)  # alpha
                admissible = np.add(action_Q, np.array(execMask))
                nextaIdex = np.argmax(admissible)
    ...
    def train(self):
        ...
        if self.samplecount >= self.minibatch_size * 3 and self.episodecount % 2 == 0:
            ...
            action_q = self.dqn.predict(s2_batch, self.epsilon_start) #alpha
            target_q = self.dqn.predict_target(s2_batch, 1.0)
            ...
            predicted_q_value, _, currentLoss, diff = self.dqn.train(s_batch, a_batch_one_hot, reshaped_yi, self.epsilon_start) #self.epsilon
