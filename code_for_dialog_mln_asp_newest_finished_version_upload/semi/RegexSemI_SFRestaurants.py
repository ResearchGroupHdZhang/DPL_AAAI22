...
"""
RegexSemI_SFRestaurants.py - regular expression based SFRestaurants SemI decoder
===============================================================
...
class RegexSemI_SFRestaurants(RegexSemI.RegexSemI):
    """
    """
    def __init__(self, repoIn=None):
        self.rTYPE = r"(restaurant|cafe)"
        
        RegexSemI.RegexSemI.__init__(self)  #better than super() here - wont need to be changed for other domains
        self.domainTag = "SFRestaurants"  #FIXME
        self.create_domain_dependent_regex() 

    def create_domain_dependent_regex(self):
        """Can overwrite any of the regular expressions set in RegexSemI.RegexSemI.init_regular_expressions().
        ...
        self.slot_vocab["pricerange"] = "(price|cost|expense|expensive|cheap)(\ ?range)*"
        self.slot_vocab["price"] = "(price|cost|expense|expensive|cheap)(?!(\ ?range))"
        self.slot_vocab["area"] = "(area|location|neighbo(u)?rhood)" 
        self.slot_vocab["near"] = "(near)" 
        self.slot_vocab["food"] = "(food|cuisine)"
        self.slot_vocab["phone"] = "(phone(\ number)*)"
        self.slot_vocab["postcode"] = "(postcode)"
        self.slot_vocab["goodformeal"] = "(meal)"
        self.slot_vocab["allowedforkids"] = "(kids|children)"
    ...
    def _set_request_regex(self):
        ...
        for slot in self.request_regex.keys():
            ...
            self.request_regex[slot] = self.rREQUEST+"\ "+self.slot_vocab[slot]+"($|\s*\?)"
            self.request_regex[slot] += "|(?<!"+self.DONTCAREWHAT+")(?<!want\ )"+self.IT+"\ "+self.slot_vocab[slot]+"($|\s*\?)"
            self.request_regex[slot] += "|(?<!"+self.DONTCARE+")"+self.WHAT+"\ "+self.slot_vocab[slot]+"($|\s*\?)"
            #self.request_regex[slot] += "|(?<!any\ )"+self.slot_vocab[slot]

        # FIXME:  Handcrafted extra rules as required on a slot to slot basis:
        self.request_regex["pricerange"] += "|(how\ much\ is\ it\ ($|\?))"
        self.request_regex["food"] += "|(what\ (type\ of\ )*"+self.slot_vocab['food']+"($|\?))"
...
#                 self.inform_regex[slot][value] += "|((what|about|which)(\ (it\'*s*|the))*)\ "+slot+"(?!\ (is\ it))" 
                #self.inform_regex[slot][value] += "|((?!(don\'*t|do\ not)\ care\ about)\ |^)"+self.slot_values[slot][value] + "($|(\ (please|and)))+"
                self.inform_regex[slot][value] += "|((?!(don\'*t|do\ not)\ care\ about)\ |^)"+self.slot_values[slot][value] + "($|(\ (please|and)))+($|\ |[^\?]$)"
...
#                     self.inform_regex[slot][value] += "|((?!(don\'*t|do\ not)\ care\ about)\ )"+self.slot_values[slot][value] + "\ food($|(\ (please|and)))+"
                if slot == "area" and value=="embarcadero":
                    self.inform_regex[slot][value] += "|(near\ ($|by|here)|local|no\ far\ away)"

            if slot == "food":
                self.inform_regex[slot]['dontcare'] = r"(any\ "+self.slot_vocab['food'] + r")|((don\'?t|do\ not)\ care\ (what|which|about|for)\ (the\ )?"+self.slot_vocab['food']+")"
                self.inform_regex[slot]['dontcare'] +=\
                        "|((don\'*t|do\ not)\ care\ (what|which|about|for)\ (the\ )*"+self.slot_vocab['food']+")"
            ...
            if slot == "area":
                LOCATION = r"(area|location|place|where|part\ of\ town)"
                self.inform_regex[slot]['dontcare'] = r"(any(\ )?"+LOCATION + ")"
                self.inform_regex[slot]['dontcare'] +=\
                        r"|((don\'*t|do\ not)\ care\ (what|which|about|for)\ (the\ )*"+LOCATION+")"
                self.inform_regex[slot]['dontcare'] += r"|any(\ *where)\ is\ (fine|ok\b|good|okay)"
                
            if slot == "goodformeal":
                MEALTYPE = "(breakfast|lunch|brunch|dinner)"
                self.inform_regex[slot]['dontcare'] = r"(any\ meal(\ type)?)"
                self.inform_regex[slot]['dontcare'] += r"|((don\'*t|do\ not)\ care\ (about)?(\ )?(which)?\ meal(\ (type|of\ the\ day)))"
                self.inform_regex[slot]['dontcare'] += r"|((don\'*t|do\ not)\ care\ about\ " + MEALTYPE + ")"
                #self.inform_regex[slot]['dontcare'] += r"|(other\ than\ " + MEALTYPE + ")"
...
#         DETECTED_SLOT_INTENT = False
        for value in self.inform_regex[slot].keys():
#             
#             #DBG
#             if slot == "food" and value=="dontcare":
#                 print "regex inform SFRestaurants for slot {} and value {}".format(slot,value)
...
#                 ADD_SLOTeqVALUE = True
                # Deal with -- DONTWANT --: does not exist yet
...
#                 if ADD_SLOTeqVALUE and not DETECTED_SLOT_INTENT:
        ...
        # process dontcare, doing this separately to hoepfully avoid messing things up            
#         value = 'dontcare'
#         if self._check(re.search(self.inform_regex[slot][value],obs, re.I)):
#             #FIXME:  Think easier to parse here for "dont want" and "dont care" - else we're playing "WACK A MOLE!"
#             ADD_SLOTeqVALUE = True
...
#                 self.semanticActs.append('inform('+slot+'='+value+')')
    ...
    def _set_value_synonyms(self):
        ...
        #---------------------------------------------------------------------------------------------------
        # TYPE:
        self.inform_type_regex = r"(restaurant|cafe)"
        # SLOT: area 
        slot = 'area'
        ...
        self.slot_values[slot]['downtown'] = "((the)\ )*(centre|center|downtown)"  # lmr46, added rule for detecting the center
        self.slot_values[slot]['dontcare'] = "any(\ )*(area|location|place|where)"
        self.slot_values[slot]['fishermans wharf'] = "(fisherman(\')?s wharf)"
        # SLOT: pricerange
        slot = 'pricerange'
        ...
        # SLOT: food
        slot = 'food'
        for value in self.slot_values[slot]:
            self.slot_values[slot][value] = "("+str(value)+"(\ (food|restaurant|place|cuisine))?)"
        
        self.slot_values[slot]['burgers'] = "(grill(\ (food|restaurant|place))?|((burger|burgers)(\ (restaurant|place))?))"
        self.slot_values[slot]['traditionnal american'] = "((traditional\ )?american(\ (food|restaurant|place|cuisine))?)"
        self.slot_values[slot]['sea food'] = "(seafood|sea\ food|fish)(\ (restaurant|place))?"
        self.slot_values[slot]['dontcare'] = "any\ (food|cuisine)"
        ...
        # SLOT: goodformeal
        self.slot_values[slot]['dontcare'] = "any\ meal"

        # SLOT: allowedforkids 
        slot = 'allowedforkids'
        #FIXME: Need to add better regular expressions
        self.slot_values[slot]['0'] = "(doesn\'?t\ | no\ | not\ )(children|kids)"
        self.slot_values[slot]['1'] = "(children|kids)(are\ )? allowed"
        #self.slot_values[slot]['0'] = "((doesn\'?t)|( (does\ )? not))\ (allow\ )? (children|kids)| (no\ ) (children|kids) "
        #self.slot_values[slot]['1'] = "((does\ )?(allow|allows)\ (children|kids)) | (are\ )? (children|kids) (are\ )? allowed"
#         self.slot_values[slot]['dontcare'] = r"(((don\'*t|do\ not)\ care\ (about\ )?(the\ )?)|no\ )(kids|children)"
        #---------------------------------------------------------------------------------------------------
