...
###############################################################################
"""
RegexSemI_SFHotels.py - regular expression based SFHotels SemI decoder
===============================================================
...
class RegexSemI_SFHotels(RegexSemI.RegexSemI):
    """
    """
    ...
    def create_domain_dependent_regex(self):
        ...
        self.slot_vocab["pricerange"] = "(price|cost|expense)(\ ?range)*"  
        ...
        self.slot_vocab["dogsallowed"] = "(dogs|pets)"
        self.slot_vocab["acceptscreditcards"] = "(((allow|accept)\ )*credit\ cards)"
...
#                 self.inform_regex[slot][value] += "|((what|about|which)(\ (it\'*s*|the))*)\ "+slot+"(?!\ (is\ it))" self.inform_regex[slot][value] += "|(\ |^)"+self.slot_values[slot][value] + "((please|and)? ($|[^\?]))"
            ...
            if slot == "pricerange":
                self.inform_regex[slot]['inexpensive'] = "(cheap|inexpensive|not\ expensive\ )"
                self.inform_regex[slot]['pricey'] = "(not\ cheap\ |expensive|pricey)"
...
#         self.slot_values[slot]['south'] = "((the)\ )*(south)"
        ...
        self.slot_values[slot]['downtown'] = "((the\ (centre|center))|downtown)"  # su259, added rule for detecting the center
        ...
        self.slot_values[slot]['inexpensive'] = "(to\ be\ |any\ )*(inexpensive|budget|cheap|bargin|cheapest|low\ cost)" #lmr46 the values in the db are: inexpensive, moderate, pricey, ultra high end
        self.slot_values[slot]['pricey'] = "(to\ be\ |any\ )*(expensive|expensively|dear|costly|pricey)"
        self.slot_values[slot]['dontcare'] = "any\ (price|price(\ |-)*range)"
        self.slot_values[slot]['ultra high end'] = "(to\ be\ |any\ )*(very expensive|very expensively|high|very\ costly\ )"
        # SLOT: near
...
#         # SLOT: dogsallowed 
        slot = 'dogsallowed'
        self.slot_values[slot]['0'] = "(dogs are not allowed|does not allow dogs|no dogs)"
        self.slot_values[slot]['1'] = "(dogs( are)? allowed|does allow dogs)"
        
#         # SLOT: acceptscreditcards 
        slot = 'acceptscreditcards'
        cards = "((credit(\ )?)?cards)"
        self.slot_values[slot]['0'] = "(("+cards+"(\ (are\ )?(not\ (allowed|accepted))))|(((isn(\')?t|doesn(\')?t|not)\ (accept|allow)(s|ing)?\ )"+cards+"))"
        self.slot_values[slot]['1'] = "(("+cards+"(\ (are\ )?(allowed|accepted)))|((?<!doesn\'t\ )(?<!doesnt\ )(?<!isn\'t\ )(?<!isnt\ )(?<!not\ )((accept|allow)(s|ing)?\ )"+cards+"))"
#         self.slot_values[slot]['dontcare'] = ""
        #---------------------------------------------------------------------------------------------------
