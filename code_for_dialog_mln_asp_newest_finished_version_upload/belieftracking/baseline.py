...
class RuleBasedTracker(BeliefTracker):
    ...
    def _updateBelief(self, turn):
        ...
        :return: None
        '''
        # convert turn info to user hypothesis by using function 'Uacts'
        track = self._addTurn(turn)
        # combine current user hypothesis with previous belief
...
#                 else:
        ...
        if self.add_knowledge:
            if self.domainString == "CamRestaurants":
                self._update_probability_cr(belief, track, slots=set(['pricerange', 'food']))
            elif self.domainString == "CamHotels":
                self._update_probability_ch(belief, track, slots=set(['pricerange', 'stars', 'kind'])) 
