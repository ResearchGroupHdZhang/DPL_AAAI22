...
class BeliefTracker(object):
    ...
    def __init__(self, domainString):
        ...
        # Settings_krr.random_krr.seed(int(self.randomseed))
        ...
        if self.add_knowledge:
            ...
            if self.mln_training_data_size:
                ...
                if self.domainString == "CamHotels":

                    init_belief_file = os.path.abspath(
                        os.path.join(
                            os.path.dirname(__file__),
                            '..',
                            # 'MLN_CH/simulated_out_marginal_{}.result'.format(self.mln_training_data_size))
                            'MLN_CH_cp/simulated_out_marginal_{}.result'.format(self.mln_training_data_size))
                    )
                    conditional_prob_file = os.path.abspath(
                        os.path.join(
                            os.path.dirname(__file__),
                            '..',
                            # 'MLN_CH/conditional_prob_{}.pkl'.format(self.mln_training_data_size))
                            'MLN_CH_cp/conditional_prob_{}.pkl'.format(self.mln_training_data_size))
                    )
            ...
            # try version
                ...
                if self.domainString == "CamRestaurants":
                    init_belief_file = os.path.abspath(
                        os.path.join(os.path.dirname(__file__), '..', 'MLN_CR/simulated_out_marginal.result')
                    )
                    conditional_prob_file = os.path.abspath(
                        os.path.join(os.path.dirname(__file__), '..', 'MLN_CR/conditional_prob.pkl')
                    )
...
# added by zzc
    ...
    def sample_external_factors():
        ...
        # shuffle
        loc = Settings_krr.random_krr.choice(possible_user_locs)
        user_loc = ['user_loc({}).'.format(loc)]
        traffic_list = list()
        weather = list()
        
        # for curr_loc in possible_user_locs:
        #     traffic_list.append('traffic({},{}).'.format(curr_loc, Settings_krr.random_krr.choice(possible_traffic, p=[0.9, 0.1])))
        # weather.append('weather({}).'.format(Settings_krr.random_krr.choice(possible_weather, p=[0.9, 0.1])))
        
        for curr_loc in possible_user_locs:
            if loc == 'west':
                if curr_loc in ['west', 'east', 'north']:
                    traffic_list.append('traffic({},{}).'.format(curr_loc,
                                                                 Settings_krr.random_krr.choice(possible_traffic, p=[0.9, 0.1])))
                else:
                    traffic_list.append('traffic({},{}).'.format(curr_loc,
                                                                 Settings_krr.random_krr.choice(possible_traffic, p=[0.1, 0.9])))
            elif loc == 'north':
                ...
                traffic_list.append('traffic({},{}).'.format(curr_loc,
                                                             Settings_krr.random_krr.choice(possible_traffic, p=[0.5, 0.5])))

        if loc == 'north' or loc == 'west':
            weather.append('weather({}).'.format(Settings_krr.random_krr.choice(possible_weather, p=[0.9, 0.1])))
        else:
            weather.append('weather({}).'.format(Settings_krr.random_krr.choice(possible_weather, p=[0.5, 0.5])))
    ...
    def _init_belief(self, constraints=None):
        ...
        # added by zzc
        # print Save_factors.external_factors_sample 
